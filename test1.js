...
//         var self = this;
//         this.onreadystatechange = oldXMLHttpRequest.onreadystatechange;
//         this.send = oldXMLHttpRequest.send;
//         actual.send = function(body){
//             self.send  = this.send;
//             oldXMLHttpRequest.send;
//         }
...
cordova.define("rs-plugin-advanced-http.http-proxy", function(require, exports, module) { var pluginId = module.id.slice(0, module.id.lastIndexOf('.'));

var cordovaProxy = require('cordova/exec/proxy');
var helpers = require(pluginId + '.helpers');

if (typeof helpers.getTypeOf !== 'function' && typeof helpers === 'function') {
  helpers = helpers();
}
...
function createXhrSuccessObject(xhr) {
  return {
    url: xhr.responseURL,
    status: xhr.status,
    data: xhr.responseType !== 'blob' && helpers.getTypeOf(xhr.responseText) === 'String' ? xhr.responseText : xhr.response,
...
function createXhrFailureObject(xhr) {
  ...
  obj.error = xhr.responseType !== 'blob' && helpers.getTypeOf(xhr.responseText) === 'String' ? xhr.responseText : xhr.response;
  obj.error = obj.error || 'Не удалось загрузить информацию. Проверьте интернет-подключение или повторите попытку позже.';
...
function sendRequest(method, withData, opts, success, failure) {
  ...
  var responseType = withData ? opts[5] : opts[4];
console.log('params');
console.log(params);
  ...
  var currentKey = url;
  var corsyUrls = ['blocks/default','/v1/', '/v2/', '/v40/'];
  isDisabled = corsyUrls.some(e => url.indexOf(e.trim()) > -1);

      if (isDisabled) {
                
          headers.orgUrl = currentKey;
        
          console.log('current key has restrictions: ',currentKey);
      }
...
//   }
  ...
  xhr.onload = function () {
    ...
    console.log('createXhrSuccessObject(xhr)');
    console.log(createXhrSuccessObject(xhr));
